...
function BannerExtension (): React.ReactElement | null {
  ...
  const phishing = useRef<string>(t('Since some extensions, such as the polkagate extension, protects you against all community reported phishing sites, there are valid reasons to use them for additional protection, even if you are not storing accounts in it.'));
  ...
  return (
    <Banner type='warning'>
      ...
      <p>{t('Accounts injected from any of these extensions will appear in this application and be available for use. The above list is updated as more extensions with external signing capability become available.')}&nbsp;
        <a
          href='https://github.com/PolkaGate/polkagate-extension'
          rel='noopener noreferrer'
          target='_blank'
        >{t('Learn more...')}</a>
      </p>
